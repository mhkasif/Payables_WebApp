...
var serviceAccount = require( __dirname + '/firebaseadminkey.json');
...
app.post('/disable-user-account', async (req, res) => {
  admin.auth().updateUser(req.body.uid,{disabled:req.body.disabled}).then(function(resp){
...
app.post('/get-cutomer-members-allowed', async (req, res) => {
  console.log(req.body.customerid);
  stripe.subscriptions.list(
    { customer: req.body.customerid },
    function (err, response) {
      var data = response.data;
      data = data.filter(function(el) {
        return (el.plan.product === process.env.USERPRODUCTID);
      });
      response.data = data;
      res.send({ response });
    }
...
app.post('/customer-add-team-member', async (req, res) => {
  console.log(req.body.customerid);
   // Create the subscription
   const subscription = await stripe.subscriptions.create({
    customer: req.body.customerid,
    items: [{ price: process.env.USERPRODUCTPRICEID
    ,quantity: req.body.member,
    }],
  ...
  //       {
  //         items:[{
  //           price: process.env.USERPRODUCTPRICEID,
  //           quantity: req.body.member,
  //         }]
  //       },
