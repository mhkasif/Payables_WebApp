...
var url =
  ...
  location.href
    ...
    .substr(0, location.href.replace(location.origin, "").lastIndexOf("/"));
  ...
  tblUsers
    ...
    .then(function (resp) {
      ...
      $(value).each(function (i, v) {
        $(".timelinePart.records." + v.substring(0, 3))
...
function userLedgerSearch(val) {
  val = val.toLowerCase();
  let list = document.querySelector("#partyNames");
  list.innerHTML = "";

  partyNameIds.forEach((item) => {
    if (item.name.toLowerCase().includes(val)) {
      list.innerHTML += `<option onclick="partyOpen()" class="party_details_tag  ${item.id}">${item.name}</option>`;
    }
  });
}

$(".balance_input").bind("input", function () {
  partyNameIds.forEach((item) => {
...
function partyOpen(id) {
  fetchPartyLedger(id);
  fetchTransactionDetial(id);
...
function filterRecordstab(tabid) {
  var checked = $(".tabcontent#" + tabid).find(
    "[name=clearPaymentCheckbox]:checked"
  );
  console.log(checked);
  if (checked.length < 1) {
    $(".tabcontent#" + tabid)
      ...
      .show();
    $(".tabcontent#" + tabid)
      .find("#acc-li-" + tabid + " tbody tr")
      .each(function (index, val) {
        $(val).show();
      });
    $(".tabcontent#" + tabid)
      .find("#acc-li-" + tabid + ">li")
      .each(function (i, v) {
        if ($(v).find('tbody>tr:not([style="display: none;"])').length > 0) {
          $(v).show();
        } else {
  ...
  $(".tabcontent#" + tabid)
    ...
    .each(function (index, val) {
      ...
      // console.log($(val).find('select > option:selected').val());
      $(".tabcontent#" + tabid)
        .find("[name=clearPaymentCheckbox]:checked")
        .each(function (i, v) {
          // console.log($(v).val());
          if ($(val).find("select > option:selected").val() === $(v).val()) {
            $(val).show();
...
function BalanceInputField() {
  ...
  $(".timelinePart").map(function () {
    ...
    $(this)
      .find(".totalBalance")
      .text("Balance Required: " + totalBalance);
  });
  ...
  if (e.keyCode === 13) {
    ...
    $(`#${id} .totalBalance`).map(function () {
      ...
      if (balanceValue > inputValue) {
        $(this)
          .next()
          .find(".alert_notification_tag")
          .prevObject.css("display", "block");
      }
