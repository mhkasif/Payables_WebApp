    ...
    firebase.auth().onAuthStateChanged(function (user) {
      if (user) {
        ...
        tblStripeCustomers
          ...
          .where("CustomerEmail", "==", localStorage.getItem("owner_email"))
          .get()
...
function AddCollaboratorGroup() {
  ...
  if ($("#group-name").val()) {
    var tbl_groups = db
      ...
      .where("UserID", "==", localStorage.getItem("userid"));
    tbl_groups
      .where("name", "==", $("#group-name").val())
      .get()
      .then(function (documentsResponse) {
        if (documentsResponse.docs.length > 0) {
          $("#group_add_message").html(
            "Team Head with same name already exist"
          );
          $("#group_add_message").show();
        } else {
          var gname = $("#group-name").val();
          var gbudget = $("#group-budget").val();
          var gfromDate = $("#group-fromdate").val();
          var gtoDate = $("#group-todate").val();
          let obj = {
            name: gname,
      ...
      querySnapshot.forEach((val)=>{
        ...
        console.log(obj);
          if (localStorage.getItem("access") == "Manager") {
            $("#gadmin").remove();
            if (obj.name == localStorage.getItem("groupid")) {
              $("#ddlGroup").append(
...
function addGroupDiv(obj) {
  var GroupDiv =
    '<div class="group_div" data-groupname="' +
    obj.name +
    '" id="group_' +
    obj.name +
    '"><div class="justify-center" style="margin-top:20px;">' +
    ' <a style="display:flex;"><h4 style="width: 100%; text-align: left; color: #949194; font-weight: 500; font-family: lato;">' +
    obj.name +
    "</h4>" +
    '     <h4 style="width: 100%; text-align: right; color: #949194; font-weight: 500; font-family: lato;"><span id="mcount_' +
    obj.name +
    '"></span> members</h4></a>' +
    //newly added budget stats for teams
    `     <div class="team-stats-container"> <div class="team-stats"><div class="team-stats-section">Total Budget<h4>${obj.budget}</h4></div> <div class="team-stats-section">Pending Budget<h4>${obj.pendingBudget = obj.pendingBudget || 0}</h4></div> <div class="team-stats-section">Spent Budget<h4>${obj.spentBudget = obj.spentBudget || 0}</h4></div> <div class="team-stats-section">Time period<h4 style="font-size: 18px;font-weight: 100;">${obj.fromDate.replace(/-/g,'/')} - ${obj.toDate.replace(/-/g,'/')}</h4></div></div>  <div class="progress"><div class="progress-bar" role="progressbar" aria-valuenow="70" aria-valuemin="0" aria-valuemax="100" style="width: 1%;"><span class="sr-only">70% Complete</span></div></div></div>` +
  ...
  if ($("#ddlGroup").val()) {
    ...
    if (viewOnly) {
      ...
      if (UserAddedAlready < TotalAllowedUsers) {
        ...
        swal(
          "You have reached the limit of alloted team members. Subscribe for additional Team Members."
        );
      }
    }
  } else {
    ...
    .then(function (res) {
      ...
      db.collection("tbl_linked_account_access")
        ...
        .set({
          ...
          group: $("#ddlGroup>option:selected").text(),
          email: email,
          islocked: false,
          owner_email: UserObject.email,
          password: password,
          type: type,
          UserID: localStorage.getItem("userid"),
        })
...
function getAccessedUsers() {
  ...
  query.get().then((querySnapshot) => {
    ...
    querySnapshot.forEach((val) => {
      ...
      td6.innerHTML =
        '<button class="settings_delete_button" style="color: #f96f6f; background: none; border: 2px solid #f96f6f; font-weight: 600;"  title="' +
        (data.islocked ? "User Locked" : "User Unlocked") +
        '">' +
        '<i class="fas fa-' +
        (data.islocked ? "lock" : "lock-open") +
        '"></i> &nbsp; ' +
        (data.islocked == true ? "Unlock" : "Lock") +
        "</button>";
      td7.innerHTML = `<button class="settings_delete_button" style="background: #f96f6f;border: 2px solid #f96f6f;"  title="Delete">
                        <i class="far fa-trash-alt"></i> &nbsp; Delete</button>`;

      viewOnly
        ? td5.setAttribute("onclick", `viewAlert();`)
        : td7.setAttribute(
            "onclick",
            `ConfirmDeleteUser(${JSON.stringify(data)})`
          );
      viewOnly
        ? td6.setAttribute("onclick", `viewAlert();`)
        : td6.setAttribute(
            "onclick",
            `ConfirmLockUnlock(${JSON.stringify(data)})`
          );
      tr.innerHTML +=
        td1.outerHTML +
        td2.outerHTML +
...
function LockUnlockUser() {
  ...
  db.collection("tbl_linked_account_access")
    .doc(data.id)
    .update({ islocked: data.islocked ? false : true })
    .then(function () {
      disableUser(data.id, data.islocked == true ? false : true).then(
        function () {
          db.collection("tbl_audit_log").add({
            content: `Member Access <b>${data.user}</b> is set to <b>${
