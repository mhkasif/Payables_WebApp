        ...
        for (var key in groupedRecords) {
            ...
            for (var record in groupedRecords[key]) {
                ...
                tblRecordsHtml += '<tr class="'+myRecord.is_signed+' ApprovalFiltersClass" id=\'' + myRecord.id + '\' data-rcdamt=\'' + myRecord.withdrawal + '\'>' +
                    ...
                    '                                <td><a href="#" onclick="editRecord(\'' + myRecord.id + '\')"> <i class="fa fa-pen"></i></a> &nbsp;<a href="#" style="color:#f46083;" onclick="deleteTrasaction(\'' + myRecord.id + '\')"> <i class="fa fa-trash"></i></a> &nbsp;<a href="#" style="color:#f46083;" <i class="fas fa-tasks"></i></a></td>' +
                    '                            </tr>';
            }
...
function updateTrasactionStatus(evt, id, newValue) {
    ...
    var withdrawalSpan = "";
    if ($(evt).parent().parent().find("td:nth-child(7)").find("span").text() == "Buyer") {

        withdrawalSpan = "<span " + (newValue === "Bounced" ? "style='text-decoration: line-through;'" : "") + ">" + $(evt).parent().parent().attr("data-rcdamt") + "</span>";
    }
    if ($(evt).parent().parent().find("td:nth-child(7)").find("span").text() == "Supplier") {
        withdrawalSpan = "<span " + (newValue === "Bounced" ? "style='text-decoration: line-through;'" : "") + ">(" + $(evt).parent().parent().attr("data-rcdamt") + ")</span>";
    }
